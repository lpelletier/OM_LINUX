...
static void inline fec_request_intrs(struct net_device *dev)
{
	...
	for (idp = id; idp->name; idp++) {
		if (request_irq
		    (b + idp->irq, fec_enet_interrupt, 0, idp->name, dev) != 0)
			printk("FEC: Could not allocate %s IRQ(%d)!\n",
			       idp->name, b + idp->irq);
	}
...
static void inline fec_request_intrs(struct net_device *dev)
{
	...
	if (request_irq(EXPIO_INT_FEC, mii_link_interrupt, 0, "fec(MII)", dev)
	    != 0)
 ...
 * map memory space (addr, addr+size) to uncachable erea.
 */
static inline unsigned long fec_map_uncache(unsigned long addr, int size)
{
	pte_t *pte;
 ...
 *  * unmap memory erea started with addr from uncachable erea.
 *   */
static inline void fec_unmap_uncache(void *addr)
{
	return;
}
